...
import { Search, Filter, Sparkles, MessageSquare, Mic, User, Brain, Trash2, Loader2 } from 'lucide-react';
...
export default function ExplorePage() {
    ...
    const handleDeleteTwin = async (twinId: string, e: React.MouseEvent) => {
        e.preventDefault();
        e.stopPropagation();
        
        if (!confirm("Are you sure you want to delete this twin? This action cannot be undone.")) return;
        
        setDeletingId(twinId);
        try {
            const response = await fetch(`${BACKEND_URL}/twins/${twinId}`, {
                method: 'DELETE',
            });
            ...
            alert("Network error. Could not delete twin.");
        } finally {
            setDeletingId(null);
        }
    };
                                        ...
                                        <div className="p-5 flex-1 flex flex-col">
                                            ...
                                            <div className="flex items-start justify-between mb-4 relative z-10">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <button 
                                                        onClick={(e) => handleDeleteTwin(twin.twin_id, e)}
                                                        disabled={deletingId === twin.twin_id}
                                                        className="w-7 h-7 rounded-lg bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 hover:border-red-500/40 text-red-400 flex items-center justify-center transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                                                        title="Delete Twin"
                                                    >
                                                        {deletingId === twin.twin_id ? (
